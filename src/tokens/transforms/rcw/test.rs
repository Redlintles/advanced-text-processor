...
mod tests {
    ...
    fn params_rejects_invalid_regex() {
        let err = Rcw::new("(", "b", 1).unwrap_err();
        assert!(!err.is_empty());
    }
    ...
    fn transform_replaces_count_occurrences_doc_example() {
        ...
        assert_eq!(t.transform("aaaaa".into(), Some(&mut ctx)).unwrap().to_string(), "bbbaa");
    }
    ...
    fn transform_with_regex_pattern_replaces_n_times() {
        ...
        assert_eq!(
            t.transform("a1 b22 c333 d4444".into(), Some(&mut ctx)).unwrap().to_string(),
            "aX bX c333 d4444"
        );
    }
