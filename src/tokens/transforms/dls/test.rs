...
mod tests {
    ...
    fn to_textforge_line_formats_correctly() {
        let t = Dls::new(7);
        assert_eq!(t.to_textforge_line().as_ref(), "dls 7;\n");
    }
    ...
    fn transform_deletes_single_char_basic_example() {
        ...
        assert_eq!(t.transform("banana".into(), Some(&mut ctx)).unwrap().to_string(), "banna");
    }
    ...
    fn transform_unicode_safe_deletes_accented_char() {
        ...
        assert_eq!(t.transform("ábc".into(), Some(&mut ctx)).unwrap().to_string(), "bc");
    }
    ...
    fn transform_unicode_safe_deletes_emoji() {
        ...
        assert_eq!(t.transform("a💥b".into(), Some(&mut ctx)).unwrap().to_string(), "ab");
    }
