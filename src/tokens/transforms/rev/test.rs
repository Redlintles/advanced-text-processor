...
mod tests {
    ...
    fn to_textforge_line_is_correct() {
        let t = Rev::default();
        assert_eq!(t.to_textforge_line().as_ref(), "rev;\n");
    }
    ...
    fn transform_reverses_ascii() {
        ...
        assert_eq!(t.transform("foobar".into(), Some(&mut ctx)).unwrap().to_string(), "raboof");
    }
    ...
    fn transform_unicode_safe() {
        ...
        assert_eq!(t.transform("áβç".into(), Some(&mut ctx)).unwrap().to_string(), "çβá");
    }
